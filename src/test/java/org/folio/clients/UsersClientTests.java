...
class UsersClientTests {
  ...
  @Test
  @SneakyThrows
  void responseFailed_isCovered_whenUsersReturnsNon200() {
    fakeWebServer.stubFor(
        get(urlPathEqualTo("/user-tenants"))
            .willReturn(aResponse().withStatus(500).withBody("downstream-error")));

    var client = new UsersClient(Headers.toMap(fakeWebServer.baseUrl()),
        WebClient.create(Vertx.vertx()));

    var future = client.getUserTenants("test-tenant");
