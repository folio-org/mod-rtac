...
class SettingsClientTests {
  ...
  @Test
  @SneakyThrows
  void responseFailed_isCovered_whenSettingsReturnsNon200() {
    fakeWebServer.stubFor(
        get(urlPathEqualTo("/settings/entries"))
            .withQueryParam("query", containing("scope==mod-rtac"))
            .willReturn(aResponse().withStatus(500).withBody("downstream-error")));

    var client = new SettingsClient(Headers.toMap(fakeWebServer.baseUrl()),
        WebClient.create(Vertx.vertx()));

    var future = client.getSettings("test-tenant", "someKey");
