...
class CirculationClient extends FolioClient {
  ...
  private static final CirculationToRtacMapper circulationToRtacMapper = new CirculationToRtacMapper();
  ...
  Future<List<InventoryHoldingsAndItems>> getLoansForItems(List<InventoryHoldingsAndItems> inventoryInstances) {
    ...
    for (InventoryHoldingsAndItems inventoryInstance : inventoryInstances) {
      ...
      for (List<Item> itemList : ListUtils.partition(items, CIRCULATION_BATCH_SIZE)) {
        ...
        futures.add(queryForLoans(httpClientRequest.copy(), inventoryInstance, cql).onSuccess(instances::addAll));
      }
    }
...
//    }
  ...
  private Future<List<InventoryHoldingsAndItems>> queryForLoans(HttpRequest<Buffer> httpClientRequest, InventoryHoldingsAndItems inventoryInstance, String cql) {
        ...
        if (ar.failed()) {
          ...
          if (i != 200) {
            ...
            final InventoryHoldingsAndItems updatedItems =
              inventoryInstance.withItems(circulationToRtacMapper.mapToRtac(loans, inventoryInstance.getItems()));
